...
const Login = () => {
  ...
  const handleLogin = async (event) => {
    ...
    try {
      const response = await axios.post("https://pharmacy-backend-k2hi.onrender.com/supplier/login", {
        email,
        password,
      });
      ...
      const reportsResponse = await axios.get(`https://pharmacy-backend-k2hi.onrender.com/report/viewReports/${supplierId}`);
